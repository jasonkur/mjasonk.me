...
$(document).ready(function() {
  ...
  $(window).scroll(function() {
    if (
      sections.filter(":in-viewport:last").attr("id") ==
      sections.last().attr("id")
    ) {
      ...
      sections.filter(":in-viewport:first").each(function() {
        ...
        var active_link = $(
          '.side-menu ul li a[href="#' + active_section.attr("id") + '"]'
        );
        menu_links.removeClass("active");
...
$(document).ready(function() {
  ...
  $(window).hashchange(function() {
    if (location.hash.search("/work") == -1 && $(".work-full").is(":visible")) {
      ...
      if (
        hash_new.replace("#/", "") != $(".work-opened").attr("href") &&
        $(".work-full").is(":visible")
      ) {
        $(".work-loader").fadeIn(300);
